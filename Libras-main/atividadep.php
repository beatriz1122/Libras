...
<html>
    ...
    <body>
        ...
        function addFileToDatabase($filename, $filetype, $filepath) {
            ...
            $stmt = $pdo->prepare('INSERT INTO ati (filename, filetype, filepath) VALUES (?, ?, ?)');
        ...
        function getFilesFromDatabase() {
            ...
            $stmt = $pdo->query('SELECT * FROM ati');
        ...
        function deleteFile($file_id) {
            ...
            $stmt = $pdo->prepare('SELECT * FROM ati WHERE id = ?');
            $stmt->execute([$file_id]);
            $file = $stmt->fetch(PDO::FETCH_ASSOC);

            if ($file) {
                $filepath = 'uploads/' . $file['filepath']; // Corrigido de 'arquivos/' para 'uploads/'
                if (file_exists($filepath)) {
                    unlink($filepath);
                }

                $stmt = $pdo->prepare('DELETE FROM ati WHERE id = ?');
                $stmt->execute([$file_id]);
            }
        }
        ...
        <form action="" method="POST" enctype="multipart/form-data">
            <input type="file" name="ati" required>
        ...
        <table>
            ...
            <?php foreach ($files as $file): ?>
                <tr>
                    ...
                    <td>
                        <a href="uploads/<?php echo htmlspecialchars($file['filepath']); ?>" target="_blank">Visualizar</a>
                        <a href="?delete=<?php echo htmlspecialchars($file['id']); ?>">Deletar</a>
                    </td>
                </tr>
            <?php endforeach; ?>
        </table>
